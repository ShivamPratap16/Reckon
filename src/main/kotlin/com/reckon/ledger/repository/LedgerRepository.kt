package com.reckon.ledger.repository

import com.reckon.ledger.enums.TxnStatus
import com.reckon.ledger.enums.TxnType
import com.reckon.ledger.model.ExistingTxn
...
class LedgerRepository(private val jdbc: JdbcTemplate) {
    ...
    fun insertPending(type: TxnType, idempotencyKey: String, requestHash: String, amount: Long, initiatorId: UUID, from: UUID, to: UUID): UUID =
        jdbc.queryForObject(
    ...
    fun findByInitiatorAndKey(initiatorId: UUID, idempotencyKey: String): ExistingTxn? = jdbc.query(
    ...
    fun markCompletedIfBankConfirmed(txnId: java.util.UUID): Int = jdbc.update(
        """UPDATE transactions SET status='COMPLETED', saga_state='DONE', updated_at=now()
           WHERE id=? AND saga_state='BANK_CONFIRMED'""",
        txnId,
    )
    ...
    fun findSagaTxnsOlderThan(sagaState: String, seconds: Long): List<java.util.UUID> = jdbc.query(
        ...
        { rs, _ -> rs.getObject("id", java.util.UUID::class.java) },
        sagaState,
        seconds.toDouble(),
    )

    fun hasNoEntries(txnId: java.util.UUID): Boolean = jdbc.queryForObject(
        "SELECT NOT EXISTS(SELECT 1 FROM ledger_entries WHERE transaction_id=?)",
        Boolean::class.java,
        txnId,
    )!!
    ...
    fun toAccountOf(txnId: java.util.UUID): java.util.UUID = jdbc.queryForObject(
        "SELECT to_account_id FROM transactions WHERE id=?",
